...
if [ -L "$0" ]; then
    DEV_SPEC_DIR="$(dirname "$(readlink "$0")")"
else
    DEV_SPEC_DIR="${DEV_SPEC_DIR:-$HOME/code/portables/devspec}"
fi

ZSH_LIB_DIR="$DEV_SPEC_DIR/zsh-lib"

# 1. Configure Oh My Zsh variables (theme, plugins, etc.)
source "$ZSH_LIB_DIR/omz-config.zsh"

# 2. Set up paths and environment initializers BEFORE sourcing Oh My Zsh
source "$ZSH_LIB_DIR/tool-config.zsh"
