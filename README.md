...
The fragment starts with an empty helper, so it replaces helpers inherited from system
configuration before applying the owned defaults.
...
include there instead. Generated files stay outside the tracked payload.

The managed include is the ownership boundary. A credential helper or conditional include before
it makes configuration fail without rewriting the managed fragment. Settings after it, including
the root machine overlay, remain explicit user overrides. Duplicate managed includes also fail;
the hook does not migrate, deduplicate, merge, or replay existing configuration. When an explicit
`GIT_CONFIG_GLOBAL` file has no credential settings or conditional includes, the hook can append
its first managed include. Repository-local configuration is outside the hook's scope. No
credentials are read, stored, or requested by this step.
