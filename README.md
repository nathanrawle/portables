...
The managed include is the ownership boundary. Before it, the hook migrates only the known
legacy settings below; an unknown credential helper or conditional include makes configuration
fail without rewriting the managed fragment. Settings after the include, including the root
machine overlay, remain explicit user overrides. Repository-local configuration is outside the
hook's scope. A repeated managed include after an override is rejected because removing it would
change that ordering. The hook does not merge or replay arbitrary helper graphs.
